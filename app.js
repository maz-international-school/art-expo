...
// THE GHOST KILLER: Wipe old search data
...
window.startVoting = async function() {
    ...
    try {
        ...
        if (settings.isGeofenceEnabled) {
            ...
            if (!position) {
                alert("Location required! Allow GPS and try again.");
                resetBtn(btn); return;
            }

            let verified = false;
            CAMPUSES.forEach(c => { if(calculateDistance(c.lat, c.lon, position.coords.latitude, position.coords.longitude) <= RADIUS_KM) verified = true; });

            if (!verified) {
                alert("Access Denied: You must be on-site to vote.");
                resetBtn(btn); return;
            }
        }
...
function setupSearch(yearId) {
    ...
    newInput.addEventListener('input', () => {
        ...
        // FILTER STICKING FIX: Must match current yearId exactly
        const matches = allArtworks.filter(a => 
            a.year === yearId && (a.id.toLowerCase().includes(val) || a.artist.toLowerCase().includes(val) || (a.title && a.title.toLowerCase().includes(val)))
        ).slice(0, 6);
        ...
        if (matches.length > 0) {
            ...
            matches.forEach(m => {
                ...
                div.onclick = () => { 
                    currentArt = m; 
                    killSearchBox(); // Wipe box on selection
                    window.confirmVote(); 
                };
